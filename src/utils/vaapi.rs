...
mod surface_pool {
    ...
    impl SurfacePool {
        ...
        pub(crate) fn set_coded_resolution(&mut self, resolution: Resolution) {
            ...
            self.surfaces
                .retain(|s| Resolution::from(s.size()).fits(self.coded_resolution));
        }
...
impl StreamMetadataState {
    ...
    fn open<S: StreamInfo>(
        ...
        let create_new_context = if !supports_context_reuse {
            true
        } else {
            match old_metadata_state {
                StreamMetadataState::Unparsed => true,
                StreamMetadataState::Parsed(ParsedStreamMetadata {
                    rt_format: old_rt_format,
                    profile: old_profile,
                    ..
                }) => va_profile != old_profile || rt_format != old_rt_format,
            }
        };

        let (create_new_surfaces, mut surface_pool) = match old_metadata_state {
        ...
        let (config, context) = if create_new_context {
            ...
            match old_metadata_state {
                ...
                _ => unreachable!(), // Otherwise we'd be creating a new context.
            }
        };
