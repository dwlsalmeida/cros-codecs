...
impl Sps {
    ...
    pub fn visible_rectangle(&self) -> Rect<u32> {
        ...
        // let sub_height_c = SUB_HEIGHT_C[usize::from(self.chroma_array_type)];
        // let sub_width_c = SUB_WIDTH_C[usize::from(self.chroma_array_type)];

        // let crop_rect_x = sub_width_c * self.conf_win_left_offset;
        // let crop_rect_y = sub_height_c * self.conf_win_top_offset;
        // let crop_rect_width = u32::from(self.width())
        //     - (self.conf_win_left_offset + self.conf_win_right_offset) * sub_width_c;
        // let crop_rect_height = u32::from(self.height())
...
impl Parser {
    ...
    /// Peek a SPS NALU. Does not overwrite parser state.
    pub fn peek_sps<T: AsRef<[u8]>>(nalu: &Nalu<T>) -> anyhow::Result<Sps> {
    ...
    fn save_sps(&mut self, sps: Sps) -> anyhow::Result<&Sps> {
